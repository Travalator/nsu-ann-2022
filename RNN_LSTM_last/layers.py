...
class FullyConnectedLayer:
    def __init__(self, n_input, n_output):
        self.W = Param(np.random.randn(n_input, n_output))
        self.B = Param(np.random.randn(1, n_output))
        self.X = None
        self.name = 'FC'
...
class RNN:
    ...
    def forward(self, input_X):
        ...
        for input_x_t, relu  in zip(self.input_X, self.relus):
            input_tanh = np.dot(input_x_t, self.W_ax.value) + np.dot(hidden, self.W_aa.value) + self.B.value
...
class LSTM:
    def __init__(self, input_size,
        ...
        # self.output_size = output_size
        self.hidden_size = hidden_size
        self.W_f = Param( np.random.randn(input_size+hidden_size, hidden_size))
        self.W_i = Param(np.random.randn(input_size+hidden_size, hidden_size))
        self.W_c = Param(np.random.randn(input_size+hidden_size, hidden_size))
        self.W_o = Param(np.random.randn(input_size+hidden_size, hidden_size))

        self.B_f = Param( np.random.randn(1, hidden_size))
        self.B_i = Param( np.random.randn(1, hidden_size))
        self.B_c = Param(np.random.randn(1, hidden_size))
        self.B_o = Param(np.random.randn(1, hidden_size))
    ...
    def backward(self, d_out):
        ...
        for input_x_t, hidden, cell_state, o_t, i_t, f_t, C_t_wave, prev_cell_state in reversed(list(zip(self.input_X, self.hidden_list[:-1], \
            ...
            d_C_t =  d_cell_state + d_out * o_t *  (1-self.tanh.forward(cell_state)**2)   # replace by tanh.backward
